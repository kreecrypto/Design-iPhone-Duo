...
mustMatch(/class="toolbar-group" role="group" aria-label="Inner display overlays"[\s\S]*?id="crease-toggle"[^>]*role="switch"[^>]*aria-checked="false"(?![^>]*disabled)[^>]*>Crease<\/button>[\s\S]*?id="app-icons-toggle"[^>]*role="switch"[^>]*aria-checked="false"[^>]*disabled[^>]*>App icons<\/button>/, 'Crease must be an enabled off-state switch after T024 preview implementation while App icons remains disabled until T025.');
