...
mustMatch(/class="toolbar-group" role="group" aria-label="Inner display overlays"[\s\S]*?id="crease-toggle"[^>]*role="switch"[^>]*aria-checked="false"[^>]*disabled[^>]*>Crease<\/button>[\s\S]*?id="app-icons-toggle"[^>]*role="switch"[^>]*aria-checked="false"[^>]*disabled[^>]*>App icons<\/button>/, 'Crease and App icons must expose off switch states without enabling unfinished interactions.');
